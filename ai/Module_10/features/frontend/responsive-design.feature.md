...
- **Active link highlight** — the active route link remains visually highlighted at all breakpoints using React Router's `<NavLink>` active class
...
- Sidebar column removed; content expands to full width
- Hamburger `[☰]` appears at the right of the top header
- User icon / action buttons remain in the header
...
- Each nav item is a full-width tap target (minimum 44px height)
...
- Same behaviour as Screen 2/3 but on a narrower viewport
- Header items may be abbreviated or overflow-hidden at very narrow widths — handle with `text-overflow: ellipsis` or by hiding secondary header items below `sm` breakpoint if needed
